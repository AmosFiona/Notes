...
- 自然段使用空一行方式进行分段
- 在导言区添加宏包\usepackeg{ctex}，或者直接使用文件类为 ctexart（两者使用其中一个，不可重复）
- 字体设置
  - latex 中字体有五种属性：字体编码，字体族，字体系列，字体形状，字体大小
  - 字体编码：正文字体编码和数学字体编码
  - 字体族：
    - 罗马字体(mainfamily->\textrm 字体设定，或\rmfamily 字体声明命令作用声明后的字体)，
    - 无衬线字体(sansfamily->\textsf)，或\sffamily 字体声明命令作用声明后的字体)，
    - 打字机字体(monofamily->\texttt)，或\ttfamily 字体声明命令作用声明后的字体)，
    - 字体按照中西亚类型分为：中亚字体(如中日韩族-->CJK)和西亚字体族(拉丁族，罗马族等)，同样具体有罗马，无衬字和打印体
    - 现代文章通常具有文字(中亚)，字母(西亚)以及公式(数学物理量)，设定字体时需要

| 中亚                            | 西亚             | 数学           |
| ------------------------------- | ---------------- | -------------- |
| \setCJKmainfont{字体名称}[参数] | \setmainfont{}[] | 通常同主地区语 |
| \setCJKsansfont{字体名称}[参数] | \setsansfont{}[] | 通常同主地区语 |
| \setCJKmonofont{字体名称}[参数] | \setmonofont{}[] | 通常同主地区语 |

- 字体系列：粗细(\bfseries)，宽度(\mdseries)
- 字体形状：直立，斜体，伪斜体(非中文)，小型大写(非中文)
- 字体大小：
- 中文字体设置（包含宏包）（中文粗体是黑体，斜体是楷书）
...
- 可选参数[]与参数名{name}或者环境名称 env{xx}之间不允许有空行，尽量保持同行或者利用%注释行功能,编译很容易出现丢失参数数字，设定为 0 的错误
- 编译过程报错提示某些字母(以字符格式输出，如`a'未能)多数是少逗号或者分号导致的
